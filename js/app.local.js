...
const Sync = {
  ...
  init() {
    ...
    try {
      this._channel = new BroadcastChannel('impulsiona_sync');
  ...
  _handleMessage(payload) {
    this._notifyListeners(payload.type, payload.data);
...
const Live = {
  ...
  start(intervalMs = 3000) {
    ...
    this._mainTimer = setInterval(() => {
      ...
      Object.keys(this._callbacks).forEach(pageId => {
  ...
  register(pageId, refreshFn) {
    if (!this._callbacks) this._callbacks = {};
    this._callbacks[pageId] = { refresh: refreshFn };
  },

  unregister(pageId) {
    if (this._callbacks) delete this._callbacks[pageId];
  },
  ...
  showUpdateBanner(message, onClick) {
    let banner = document.getElementById('live-update-banner');
    ...
    banner.style.cssText = `
      ...
      cursor:pointer; z-index:9999; font-size:0.9rem; font-weight:600;
...
const DB = {
  get(key) {
    try { return JSON.parse(localStorage.getItem('impulsiona_' + key)) || null; }
    catch { return null; }
  },
  set(key, value) {
...
const Auth = {
  ...
  deleteAccount() {
    ...
    if (user.type === 'company') {
      const jobs = DB.getJobs().filter(j => j.companyId !== user.id);
      DB.setJobs(jobs);

      const apps = DB.getApplications().filter(a => {
        const job = (DB.getJobs() || []).find(j => j.id === a.jobId);
        return job && job.companyId !== user.id;
      });
...
const Jobs = {
  getAll(filters = {}) {
    ...
    if (filters.regime) {
      jobs = jobs.filter(j => j.regime === filters.regime);
    }
    if (filters.type) {
      jobs = jobs.filter(j => j.type === filters.type);
    }
    if (filters.salaryMin) {
      jobs = jobs.filter(j => j.salaryMax >= parseInt(filters.salaryMin));
    }
  ...
  delete(id) {
    ...
    const msgs = DB.getMessages().filter(m => m.jobId !== id);
    DB.setMessages(msgs);
  ...
  getStats(companyId) {
    ...
    return {
      ...
      totalApplications: apps.filter(a => jobs.some(j => j.id === a.jobId)).length,
      pendingApplications: apps.filter(a => jobs.some(j => j.id === a.jobId) && a.status === 'pending').length,
    };
  },
  ...
  getStatsGlobal() {
    ...
    return {
      totalJobs: jobs.length,
      totalCompanies: DB.getUsers().filter(u => u.type === 'company').length,
      totalCandidates: users.length,
      totalApplications: DB.getApplications().length,
    };
  }
};
...
const Resume = {
  ...
  save(data) {
    ...
    if (existing >= 0) {
      resumes[existing] = { ...resumes[existing], ...resume };
    } else {
      resume.id = DB.generateId();
      resume.createdAt = new Date().toISOString().split('T')[0];
      resumes.push(resume);
    }
  ...
  getCompletion(resume) {
    ...
    check(resume.name); check(resume.email); check(resume.phone);
    check(resume.title); check(resume.summary);
    check(resume.experience); check(resume.education);
    check(resume.skills); check(resume.languages);
...
const Applications = {
  apply(jobId, resumeId, coverLetter, pdfData, pdfName) {
    const apps = DB.getApplications();
    const user = Auth.getUser();
    if (apps.find(a => a.jobId === jobId && a.userId === user.id)) {
    ...
    const app = {
      ...
      resumePdf: pdfData || null,
      resumePdfName: pdfName || '',
      status: 'pending',
  ...
  getByCompany(companyId) {
    const jobs = Jobs.getCompanyJobs(companyId);
    const jobIds = jobs.map(j => j.id);
    return DB.getApplications().filter(a => jobIds.includes(a.jobId));
  },
  ...
  updateStatus(appId, status) {
    ...
    if (idx >= 0) {
      ...
      DB.setApplications(apps);
      Notifications.add(apps[idx].userId, `Sua candidatura foi atualizada para: ${getStatusText(status)}`);
      return apps[idx];
    }
    return null;
  },
  ...
  hasApplied(jobId, userId) {
    return DB.getApplications().some(a => a.jobId === jobId && a.userId === userId);
  },
  ...
  delete(appId) {
    const apps = DB.getApplications().filter(a => a.id !== appId);
    DB.setApplications(apps);
  }
};
...
const Messages = {
  send(from, to, jobId, text) {
    ...
    if (sender) {
      Notifications.add(to, `Nova mensagem de ${sender.name}: "${text.substring(0, 60)}${text.length > 60 ? '...' : ''}"`);
    }
  ...
  getUnreadCount(userId) {
    return DB.getMessages().filter(m => m.to === userId && !m.read).length;
  },

  markRead(userId, otherId, jobId) {
    const msgs = DB.getMessages();
    msgs.forEach(m => {
      if (m.to === userId && m.from === otherId && m.jobId === jobId && !m.read) {
        m.read = true;
      }
    });
    DB.setMessages(msgs);
  ...
  getConversations(userId) {
    ...
    msgs.forEach(m => {
      if (m.from === userId || m.to === userId) {
        ...
        if (!convos[key] || new Date(m.timestamp) > new Date(convos[key].timestamp)) {
          convos[key] = { otherId, jobId: m.jobId, text: m.text, timestamp: m.timestamp, read: m.read };
        }
      }
    });
  ...
  deleteByJob(jobId) {
    const msgs = DB.getMessages().filter(m => m.jobId !== jobId);
    DB.setMessages(msgs);
  },

  deleteConversation(userId, otherId, jobId) {
    const msgs = DB.getMessages().filter(m =>
      !(m.jobId === jobId && ((m.from === userId && m.to === otherId) || (m.from === otherId && m.to === userId)))
    );
    DB.setMessages(msgs);
  }
};
...
const Notifications = {
  ...
  getByUser(userId) {
    return DB.getNotifications().filter(n => n.userId === userId)
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  },
...
function getStatusText(status) {
  const map = { pending: 'Pendente', viewed: 'Visualizado', accepted: 'Aceito', rejected: 'Rejeitado', cancelled: 'Cancelado' };
  return map[status] || status;
}

function getStatusClass(status) {
  const map = { pending: 'tag-orange', viewed: 'tag-blue', accepted: 'tag-green', rejected: 'tag-red', cancelled: 'tag-gray' };
...
function getCategoryIcon(cat) {
  const map = { 'Tecnologia': '💻', 'Marketing': '📢', 'Design': '🎨', 'Gestão': '📊', 'Financeiro': '💰', 'Saúde': '🏥', 'Educação': '📚', 'Engenharia': '⚙️', 'Jurídico': '⚖️', 'Vendas': '🤝', 'RH': '👥', 'Logística': '🚚' };
  return map[cat] || '💼';
}

function getRegimeIcon(r) {
  const map = { 'Remoto': '🏠', 'Híbrido': '🔄', 'Presencial': '🏢' };
  return map[r] || '📍';
}
...
function renderNavbar() {
  ...
  let navLinks = '';
  navLinks += `<a href="index.html">Início</a>`;
  navLinks += `<a href="jobs.html">Vagas</a>`;

  if (!user) {
    navLinks += `<a href="login.html" class="btn-nav">Entrar</a>`;
    navLinks += `<a href="register.html" style="background:rgba(255,255,255,0.15);">Cadastrar</a>`;
  } else if (user.type === 'candidate') {
    navLinks += `<a href="candidate-dashboard.html">Dashboard</a>`;
    navLinks += `<a href="my-applications.html">Candidaturas</a>`;
  } else {
    navLinks += `<a href="company-dashboard.html">Dashboard</a>`;
    navLinks += `<a href="company-jobs.html">Minhas Vagas</a>`;
    navLinks += `<a href="create-job.html" class="btn-nav">+ Nova Vaga</a>`;
  }

  let userMenu = '';
  if (user) {
    const notifBadge = unreadNotifs > 0 ? `<span style="background:#dc3545;color:#fff;border-radius:50%;padding:1px 6px;font-size:0.65rem;position:absolute;top:-4px;right:-4px;">${unreadNotifs}</span>` : '';
    userMenu = `
      <div class="nav-dropdown">
        <div class="nav-user" onclick="toggleDropdown(this)">
        ...
        <div class="nav-dropdown-menu" id="user-dropdown">
          ${user.type === 'candidate' ? `
            <a href="candidate-dashboard.html">Meu Painel</a>
            <a href="resume.html">Meu Currículo</a>
            <a href="my-applications.html">Candidaturas</a>
            <a href="messages.html">Mensagens ${unreadMsgs > 0 ? `<span class="badge badge-urgent" style="margin-left:4px;">${unreadMsgs}</span>` : ''}</a>
            <a href="notifications.html">Notificações ${unreadNotifs > 0 ? `<span class="badge badge-urgent" style="margin-left:4px;">${unreadNotifs}</span>` : ''}</a>
          ` : `
            <a href="company-dashboard.html">Meu Painel</a>
            <a href="company-jobs.html">Minhas Vagas</a>
            <a href="company-applications.html">Candidaturas Recebidas ${unreadMsgs > 0 ? `<span class="badge badge-urgent" style="margin-left:4px;">${unreadMsgs}</span>` : ''}</a>
